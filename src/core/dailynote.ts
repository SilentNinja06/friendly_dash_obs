...
/**
 * The daily-note contract. Every write to the daily note goes through this
 * module. Rules that live here and nowhere else:
 *
 *  - Path/format/template come from the *core Daily Notes plugin's own options*
 *    — never hardcoded. So the dashboard always writes to the same note the user
 *    (and other plugins) use.
 *  - Writes use `app.vault.process()` (atomic read-modify-write) — never a
 *    read-then-write, which races Obsidian Sync and a user edit (LANDMINE).
 *  - If today's note is open in another leaf, we reconcile against the *live
 *    editor content*, editing only the target range, instead of clobbering the
 *    file on disk under the editor (LANDMINE). No duplicate content, no lost
 *    keystrokes.
 *  - Heading matchers are colon-free (LANDMINE): the template ships without
...
/** Get (or create, from the core template) today's daily note. Never creates a
 * second note for the day — the path is resolved from the plugin's options. */
...
/** Apply `transform` to today's note. If the note is open in an editor, edit the
 * live buffer in place (reconcile — no disk clobber under the editor); otherwise
 * use vault.process (atomic, Sync-safe). */
...
/** Write the body of a free-text field into today's note. */
...
/** Read the current body of a free-text field from today's note — from the live
 * editor if open, else from disk. */
