 ...
 * from the dropdown, or a new one typed in) right from the creation modal. The
 * note is created empty — the filename is the title, no duplicate heading. */
export class NewNoteModal extends Modal {
	...
	onOpen(): void {
		...
		new Setting(this.contentEl)
			.setName("Category")
			.setDesc("Optional — file it under a category as you create it.")
			.addDropdown((dd) => {
				dd.addOption("", "(none)");
		...
		new Setting(this.contentEl)
			...
			.setDesc("Creates the category and files this note under it.")
	...
	private async submit(): Promise<void> {
		...
		if (!title) {
			new Notice("Please give the note a title.");
			return;
		}
...
export class NewCategoryModal extends Modal {
	...
	private async submit(): Promise<void> {
		...
		if (!name) {
			new Notice("Please give the category a name.");
			return;
		}
...
export function runAssignFlow(app: App, store: LibraryStore, onDone: () => void): void {
	...
	if (notes.length === 0) {
		new Notice("There are no notes to file yet. Make one first.");
		return;
	}
	new NoteSuggestModal(app, notes, (note) => {
		...
		new CategoryPromptModal(app, cats, async (category) => {
			...
			new Notice(`Filed “${note.basename}” under “${category}”.`);
			onDone();
		}).open();
	}).open();
}
...
class CategoryPromptModal extends Modal {
	...
	onOpen(): void {
		...
		new Setting(this.contentEl)
			.addButton((b) => b.setButtonText("Cancel").onClick(() => this.close()))
			.addButton((b) => b.setButtonText("File it").setCta().onClick(() => this.submit()));
	}
	private submit(): void {
